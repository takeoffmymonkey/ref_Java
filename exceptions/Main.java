...
@Ntrstn("Компилятор требует обрабатывать только проверяемые (компилятором) исключения - т.е. те, " +
        ...
        "или выход за границы массива, такие нужно исправлять в коде, а не обрабатывать")
@Ntrstn("У блока try обязательно должен быть либо catch либо finally, либо оба")
@Ntrstn("Между блоками try и catch не может быть другого кода")
@Ntrstn("Между блоками try и finally может быть только код catch")
@Ntrstn("Выполнение кода в try прерывается после возникновения исключения")
@Ntrstn("Обработчик catch пропускается в 3 случая: если в try не возникло исключения, если тип " +
...
@Ntrstn("Блок finally выполняется почти всегда - даже без возникновения исключения или без его " +
        "обработки. Но может не выполниться, если JVM выйдет не дойдя до него, или будет прерван " +
        "или убит поток (а программа может при этом продолжать работу)")
@Ntrstn("Блоков finally у try может быть не больше одного, в отличие от catch")
@Ntrstn("На пути поиска подходящего типа обработчика используется первый подходящий, в т.ч. " +
        ...
        "непроверяемое)")
@Ntrstn("Выбросить исключение можно в блоке catch, напр. чтобы переопределить его тип")
...
@Ntrstn("Компилятор требует ему сообщать только о выбрасывании проверяемых исключений, т.е. если в " +
        "теле метода есть бросание непроверяемого исключения, то об этом даже не обязательно " +
        "объявлять перед телом метода")
...
@Ntrstn("Указывать выбрасывыемые исключения, которые на самом деле метод не выбрасывает, можно чтобы " +
        "была возможность добавить их в будущем")
...
@Ntrstn("По сути это способ попытаться восстановиться (исправлением ситуации) от ошибки, выйдя из " +
        "метода, в котором она произошла")

@Ntrstn("Конструктор волен выкинуть любое исключение, не обращая внимание на то, какие исключения " +
        "производить родительский. Но он должен все равно указать все родительские")

@Ntrstn("Несмотря на то что компилятор заставлет описывать исключения при наследовании, это не " +
        "является частью объявления метода. Интерфейс спецификации исключений для определенного " +
        "метода может сузиться в процессе наследования и переопределения, но не расшириться - это " +
        "прямая противоположнотсь интерфейсу класса во время наследования")

@Ntrstn("Если сбой в конструкторе произойдет до того, как он будет полностью выполнен, то он может " +
        "не успеть создать некоторую часть объекта, освобождаемого в finally. Нужно использовать " +
        "вложенные блоки try")
