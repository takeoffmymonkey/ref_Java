...
@Ntrstn("try-with-resources заметно сокращает и упрощает работу с ресурсами")
@Ntrstn("Дополнительные блоки у try-with-resources (catch или finally) срабатывают после закрытия " +
        "ресурсов")
@Ntrstn("try-with-resources может быть вообще без дополнительных блоков, но т.к. метод close() " +
        "ресурса выбрасывает исключение, его все равно придется где-нибудь обрабатывать")
@Ntrstn("В скобках у try-with-resources можно только объявить и сразу инициализировать объект " +
        "ресурса - нельзя только объявить ссылку, только инициализировать объект или использовать " +
        "ссылку на существующий ресурс")
