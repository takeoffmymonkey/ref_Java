 ...
 *      - java.nio.charset (description): наборы символов, кодеры/декодеры для перевода между
 ...
 *      - Serializable: имплементирующий класс можно (де)сериализировать
 *      - FileFilter: фильтр для абстрактных имен путей
 *      - DataInput|DataOutput: чтение/запись байтов в/из бинарного потока и реконструкция из них
 *      примитивов
 *          - ObjectInput|ObjectOutput: также чтение/запись объектов
 *
 * - основан на паттерне Декоратор
 *      - т.е. функцонал добавляется оборачиванием объектов в новый класс */
...
@Ntrstn("IO, NIO и NIO.2 не являются заменой друг другу, а скорее расширением существующего функционала")
