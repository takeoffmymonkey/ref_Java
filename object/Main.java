...
/* ~~~~~~~~~~~~~~~~~~~~~~~ПОТОКОВЫЕ МЕТОДЫ (ВЗАИМОДЕЙСТВИЯ МЕЖДУ ПОТОКАМИ)~~~~~~~~~~~~~~~~~~~~~~~*/
/* WAIT()
 * - public final void wait(), wait(long timeout), wait(long timeout, int nanos): указывает ветке
 * приостановить выполнение, отдать монитор и ожидать уведомления от notifyAll(), а после его
 * получения запросить монитор обратно и продолжить выполение с места остановки
 *      - для работы нужен монитор на какой-то объект
 *      - может устанавливать тайм-аут ожидания (с дополнительной точностью в наносекундах) */

/* NOTIFYALL(), NOTIFY()
 * - public final void notifyAll(): сообщает ожидающим веткам (остановлены методом wait) о
 * наступлении важного события
 *      - версия notify() предназначена для приложения с множеством веток, делающих схожую работу,
 *      где неважно, какая из веток будет запрашивать монитор
 *      - для работы нужен монитор на тот же объект, что и для wait */
...
@Ntrstn("Многопоточные методы wait/notifyAll используются в связке для общения между 2 ветками, " +
        "которые работают с данным объектом. Метод wait приказывает ветке отпустить замок и ожидать " +
        "уведомления от другой ветки (от вызова метода notifyAll там) и, при получении, снова " +
        "запросить замок и продолжить с места остановки. Версия notify() предназначена для приложения " +
        "с множеством веток, делающих схожую работу, где неважно, какая из веток запросит монитор")
