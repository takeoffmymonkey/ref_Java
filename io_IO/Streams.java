 ...
 *      - напр. new BufferedReader(new FileReader("foo.in"));
 *          -  буферизирует вход из указанного файла
 *              - без буферизации, каждый вызов read() или readLine() приводил бы к тому, что
 *              байты читались из файла, конвертировались в символы, а затем возвращались, что было
 *              бы очень не эффективным
 *
...
@Ntrstn("Поток - последовательность данных, считываемая из источника или записываемая в цель: " +
        "[ИСТОЧНИК ДАННЫХ] -> (входящий поток) -> [ПРОГРАММА] -> (исходяший поток) -> [ЦЕЛЬ ДАННЫХ]")

@Ntrstn("Источником/целью могут быть разные сущности, типа файлов, дисков, устройств, массивы памяти " +
        "и т.д.")

@Ntrstn("Все данные внутри потоков представлены как байты, при этом передавать можно как байты, " +
        "так и символы (символьная иерархия, см. ниже), примитивы при сериализации (DataInputStream/" +
        "DataOutputStream) и объекты при сериализации (DataInputStream/DataOutputStream или " +
        "ObjectInputStream/ObjectOutputStream)")

@Ntrstn("Для представления потоков существуют 2 большие иерархиями классов: байтовые потоки " +
        "(InputStream/OutputStream) и символьные потоки (Reader/Writer - просто для удобства; " +
        "оборачивают байтовые потоки)")

@Ntrstn("В отличие от каналов, потоки являются однонаправленными (1 для записи, другой для чтения) " +
        "и после закрытия перестают существовать (канал может быть закрытым). Потоки всегда " +
        "необходимо закрывать (как и каналы) - либо в блоке finally, либо в блоке try-with-resources")

@Ntrstn("Функционал к потоку добавляется оборачиванием (паттерн Декоратор), напр: new " +
        "BufferedReader(new FileReader(1.txt)) - значит класс, читающий посимвольно файлы, " +
        "передается в класс, читающий символы при помощи буфера, и получается класс, читающий " +
        "посимвольно файлы при помощи буфера. FilterInputStream/FilterOutputStream - родители для " +
        "Декораторов")

@Ntrstn("Передача данных побайтово является очень неэффективной, поэтому с введением NIO (где по " +
        "дефолту используется буфер), классы потоков получили возможность также использовать буферы " +
        "для сокращения количества операций I/O. Добавлены BufferedInputStream и BufferedOutputStream " +
        "для байтовых иерархий и BufferedReader и BufferedWriter для символьных. Входной поток " +
        "считывает данные из буфера в памяти, когда буфер заполнен, выходной поток записывает данные " +
        "из буфера, когда он заполнен. Передача происходит обычными методами read/write")

@Ntrstn("Классы InputStreamReader/OutputStreamWriter можно использовать как более обобщенные мосты " +
        "байт-в-символ, кроме существующих Reader/Writer (которые также являются такими мостами)")

@Ntrstn("Существует утилитный класс java.nio.channels.channels, который предоставляет методы для " +
        "перевода из потока в канал и обратно")
