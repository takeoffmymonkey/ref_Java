...
@Ntrstn("Класс java.io.File является представлением пути к файлу или папке в файловой системе (в NIO2 " +
        "заменен на java.nio.file.Path). При этом реального файла/папки по данному пути может (еще) " +
        "не быть.")

@Ntrstn("Путь может быть абсолютый (содержит всю информацию для находжения файла, который он " +
        "обозначает) и относительный (для полноты необходимо добавить дополнительную информацию из " +
        "другого пути)")

@Ntrstn("Имена папок/файлов в пути разделяются разделителем, который зависит от системы (и доступен " +
        "в статических полях separator и separatorChar)")

@Ntrstn("По дефолту все классы в java.io всегда разрешают пути относительно текущей папки пользователя" +
        "(обычно там, откуда запущена JVM)")

@Ntrstn("Основные методы класса java.io.File можно разделить на следующие группы: " +
        "1 - проверка файла на различные способности и атрибуты" +
        "2 - создание" +
        "3 - удаление" +
        "4 - переименование" +
        "5 - получение данных от файла (обычно названия или пути)" +
        "6 - установка для файла атрибутов и времени изменения" +
        "7 - получение списка других файлов, как-то относящихся к данному" +
        "8 - получение данных о системных дисках и месте на диске" +
        "9 - перевод файла в другое представление, например toPath()")

@Ntrstn("С J7 пакет java.nio.file (NIO2) определяет интерфейсы и классы для JVM для операций доступа " +
        "к файлам, файловым атрибутам и файловой системе. Он позволяет преодолевать многие " +
        "ограничения класса File - класс File по сути заменен на Path, а работа с ним идет в классе " +
        "Files. Есть специальный метод toPath, который переводит объект File в Path")
