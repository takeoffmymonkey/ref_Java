...
@Ntrstn("Механизм рефлексии является нетрадиционным способом получения информации о типах класса, и, " +
        ...
        "при помощи ручного создания объекта почти любого класса, вызова любого метода и изменения " +
...
@Ntrstn("Рефлексия не используется часто в обычных программах, но она не заменима в случаях, когда " +
...
@Ntrstn("К минусам рефлексии относятся:" +
        ...
        "- нарушение инкапсуляции (т.к. доступно все, нарушается смысл класса как такового)")

@Ntrstn("Все вспомогательные классы, представляющие конструкции языка (например, ключевые классы " +
        "Field, Method, Constructor, Parameter, Modifier, Type, TypeVariable, Array), находятся в пакете java.lang.reflect, но " +
        "не имеют публичных конструкторов, и либо предоставляют статические утилитные методы " +
        "(например, Array или Modifier - напр. перекодирует число в модификатор или константу, " +
        "представляющую модификатор) либо создаются от экземпляра класса Class (находится в пакете " +
        "java.lang) - например, соответствующими методами getFields(), getMethods(), getConstructors(). " +
        "Класс Class можно смело считать входной точкой в API рефлексии, хоть он сам и не " +
        "принадлежит пакету рефлесии.")
...
@Ntrstn("У каждого загруженного класса в программе, есть соответствующий экземпляр класса Class " +
        ...
        "т.к. это уже готовая ссылка. Не приводит к созданию объекта Class, если данный класс еще " +
        "не был создан." +
        "2 - объект.getClass(): когда есть нужный объект. Работает только для ссылочных типов. Если " +
        "соответствующий объект класса еще не был создан, то он создастся" +
...
@Ntrstn("Также ссылку на экземпляр Class можно получить и от другого Class: на класс-родитель, на " +
        "обрамляющий класс, на все члены-классы данного класса, на класс, в котором объявлен данный")
...
@Ntrstn("Получать члены можно при помощи соответствующего метода класса Class либо по имени члена " +
        "(например, getField(String name)) либо массивом (например, getFields()) - будет возвращен " +
        "объект или массив объектов соответствующего класса (реализация интерфейса Member), например " +
        "Field.")
...
@Ntrstn("У методов get/set классов Field и Array есть сходства: можно считывать/записывать " +
        ...
        "общую версию для Object. Во втором случае, если достается примитив, то он будет обернут в " +
        "Object, а если нужно записать примитив, то он передается как Object, а затем автоматически " +
        "распаковывается перед записью. При этом важно помнить, что в рефлексии не происходит " +
        "автоупаковка и автораспаковка примитивов из собственных их оберток (т.к. она происходит при " +
        "компиляции), т.е. если я получаю " +
        "примитивное значение int из поля, я не могу его сразу записывать в объект Integer, и " +
        "наоборот. Кроме того, если пытаться записать финализированное поле, то будет выброшено " +
        "исключение. И еще один нюанс - методы первым аргументом принимают объект, над которым " +
        "производится действие, а если это статический класс, то объект будет просто проигнорирован, " +
        "можно передавать спокойно null")
...
@Ntrstn("Класс Executable является прямым родителем Method и Constructor. В качестве параметра для " +
        ...
        "также параметров типа. Кроме того, можно узнать, является есть ли у метода/конструктора " +
...
@Ntrstn("Методы класса Class работают для всех ссылочных типов, включая массивы и перечисления. " +
        ...
        "массивов и java.lang.reflect.Array для массивов")
